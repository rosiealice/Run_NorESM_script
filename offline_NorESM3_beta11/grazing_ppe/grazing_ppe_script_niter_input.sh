...
# Optional first argument: niter (must be a non-negative integer).
# Defaults to 1 if not provided.
niter="${1:-1}"
if ! [[ "$niter" =~ ^[0-9]+$ ]]; then
    echo "Error: niter must be a non-negative integer."
    echo "Usage: $0 [niter]"
    exit 1
fi

resolution="f45_f45_mg37" #f19_g17, ne30pg3_tn14, f45_f45_mg37, ne16pg3_tn14 a
casename="grazing_ppe_$niter"
...
then
    ...
    rm -rf /cluster/work/users/rosief/noresm/$casename
        if [[ -d "$casedir" ]] 
        then    
...
#        cp /cluster/home/kjetisaa/Trendy_2025_scripts/SourceModFiles_S3rerun/BalanceCheckMod.F90 $workpath$casename/SourceMods/src.clm/
	...
	echo " use_fates_nocomp = .true." >> $casedir/user_nl_clm
 	echo " use_fates_fixed_biogeog = .true." >> $casedir/user_nl_clm
        echo " fsurdat='/cluster/shared/noresm/inputdata/lnd/clm2/surfdata_esmf/ctsm5.4.0/surfdata_4x5_hist_2000_16pfts_c251022.nc'" >> $casedir/user_nl_clm        
        echo "fates_paramfile ='/cluster/work/users/rosief/paramscratch/grazing_ppe/grazing_ppe_$niter.nc'" >> $casedir/user_nl_clm
